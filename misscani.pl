...
oper(leva3, [M,C1,0], [M,C2,1]) :- C2 is (C1-2), C1 > 1.
oper(leva4, [M1,C1,0], [M2,C2,1]) :- M2 is (M1-1), C2 is (C1-1), M1 > 0, C1 > 0.
...
oper(volta3, [M,C1,1], [M,C2,0]) :- C2 is (C1+2), C1 < 2.
oper(volta4, [M1,C1,1], [M2,C2,0]) :- M2 is (M1+1), C2 is (C1+1), M1 < 3, C1 < 3. 
